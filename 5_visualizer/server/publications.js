...
Meteor.publish('topArtists', function(count, category) {
    ...
    // build filter
    var filter = { 'sort' : {},  'limit' : parseInt(5*count)};
...
Meteor.publish('nodes', function (artistId) {
    ...
    console.log("fetchNetworknodes", fetchNetworkNodes.length);
    var nodes = Nodes.find({ 'data.id' : { "$in": fetchNetworkNodes.map(function (d) {
        return String(d);
    }) } });
